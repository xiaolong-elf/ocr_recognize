...
def pad_group_image(old_im, output_path, buckets):
    ...
    if j != -1:
        ...
        assert new_im.shape[0] == 32 and new_im.shape[1] <= 1200
        cv2.imwrite('%s' % output_path, new_im)


path = '../../data/baidu_data/baidu'
output_path = '../../data/baidu_data/process_image/'
...
for i in image_list:
    ...
    image = cv2.imread(i)
    image = resize(image)
    pad_group_image(image, new_path, buckets=buckets_size)
