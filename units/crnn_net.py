...
'''
# filter_height or filter_weight is kernel size.
'''
...
def pool_layer(bottom, wpool, padding, name):
    ...
    top = tf.layers.max_pooling2d(bottom, [2, 2], [2, wpool],
                                  padding=padding,
                                  name=name)
    return top
...
def convnet_layers(inputs, widths, mode):
    ...
    # inputs should have shape [ ?, 46, 46, 1 ]
    with tf.variable_scope("convnet"):  # h,w

        conv1 = conv_layer(inputs, layer_params[0], training)  # 44,44
        conv2 = conv_layer(conv1, layer_params[1], training)  # 44,44
        pool2 = pool_layer(conv2, 2, 'valid', 'pool2')  # 22,22
        conv3 = conv_layer(pool2, layer_params[2], training)  # 22,22
        conv4 = conv_layer(conv3, layer_params[3], training)  # 22,22
        ...
        int_two = tf.constant(2, dtype=tf.int32)
        after_conv1 = tf.subtract(widths, int_two)
        after_conv1 = tf.cast(after_conv1, tf.float32)
...
def rnn_layers(features, sequence_length, rnn_size):
